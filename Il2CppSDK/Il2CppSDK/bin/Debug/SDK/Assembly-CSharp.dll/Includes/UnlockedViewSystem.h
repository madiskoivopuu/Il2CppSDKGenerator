#pragma once
#include <Il2Cpp/Il2Cpp.h>

class UnlockedViewSystem
{
public: 

	static Il2CppClass *StaticClass() {
		return (Il2CppClass *)(Il2Cpp::GetClass("Assembly-CSharp.dll", "", "UnlockedViewSystem"));
	}

	template <typename T = ArenaUnityWorld*> T& _world() {
		return *(T*)((uintptr_t)this + 0x28);
	}

	template <typename T = ICollector1GameEntity*>*> T GetTrigger(IContext1GameEntity*>* context) {
		return ((T (*)(UnlockedViewSystem*, IContext1GameEntity*>*))(Il2CppBase() + 0x153B328))(this, context);
	}
	template <typename T = bool> T Filter(GameEntity* entity) {
		return ((T (*)(UnlockedViewSystem*, GameEntity*))(Il2CppBase() + 0x153B3F4))(this, entity);
	}
	template <typename T = void> T Execute(Il2CppList<GameEntity*>* entities) {
		return ((T (*)(UnlockedViewSystem*, Il2CppList<GameEntity*>*))(Il2CppBase() + 0x153B410))(this, entities);
	}

};

