#pragma once
#include <Il2Cpp/Il2Cpp.h>

class ChangeFollowTargetSystem
{
public: 

	static Il2CppClass *StaticClass() {
		return (Il2CppClass *)(Il2Cpp::GetClass("Assembly-CSharp.dll", "", "ChangeFollowTargetSystem"));
	}

	template <typename T = ArenaUnityWorld*> T& _world() {
		return *(T*)((uintptr_t)this + 0x28);
	}

	template <typename T = ICollector1GameEntity*>*> T GetTrigger(IContext1GameEntity*>* context) {
		return ((T (*)(ChangeFollowTargetSystem*, IContext1GameEntity*>*))(Il2CppBase() + 0x13EA3AC))(this, context);
	}
	template <typename T = bool> T Filter(GameEntity* entity) {
		return ((T (*)(ChangeFollowTargetSystem*, GameEntity*))(Il2CppBase() + 0x13EA404))(this, entity);
	}
	template <typename T = void> T Execute(Il2CppList<GameEntity*>* entities) {
		return ((T (*)(ChangeFollowTargetSystem*, Il2CppList<GameEntity*>*))(Il2CppBase() + 0x13EA420))(this, entities);
	}

};

