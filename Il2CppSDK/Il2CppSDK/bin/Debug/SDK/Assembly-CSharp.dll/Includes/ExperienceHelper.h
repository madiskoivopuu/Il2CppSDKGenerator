...
public: 
	...
	template <typename R = int32_t> static R GetTotalLevelCap(AccountEntity* account) {
	 ...
	 static ValueTuple2<uint32_t, uint32_t>* GetProgress(Il2CppArray<uint32_t>* levels, float level) {
		return ((ValueTuple2<uint32_t, uint32_t>* (*)(void *, Il2CppArray<uint32_t>*, float))(Il2CppBase() + 0x1685270))(0, levels, level);
	}
	 static ValueTuple2<uint32_t, uint32_t>* GetConvertProgress(float level) {
		return ((ValueTuple2<uint32_t, uint32_t>* (*)(void *, float))(Il2CppBase() + 0x168536C))(0, level);
	}
