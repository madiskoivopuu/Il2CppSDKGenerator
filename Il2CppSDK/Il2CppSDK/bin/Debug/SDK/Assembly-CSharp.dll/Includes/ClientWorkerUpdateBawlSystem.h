#pragma once
#include <Il2Cpp/Il2Cpp.h>

class ClientWorkerUpdateBawlSystem
{
public: 

	static Il2CppClass *StaticClass() {
		return (Il2CppClass *)(Il2Cpp::GetClass("Assembly-CSharp.dll", "", "ClientWorkerUpdateBawlSystem"));
	}

	template <typename T = ArenaClientWorld*> T& _world() {
		return *(T*)((uintptr_t)this + 0x28);
	}

	template <typename T = ICollector1GameEntity*>*> T GetTrigger(IContext1GameEntity*>* context) {
		return ((T (*)(ClientWorkerUpdateBawlSystem*, IContext1GameEntity*>*))(Il2CppBase() + 0xE22038))(this, context);
	}
	template <typename T = bool> T Filter(GameEntity* entity) {
		return ((T (*)(ClientWorkerUpdateBawlSystem*, GameEntity*))(Il2CppBase() + 0xE22090))(this, entity);
	}
	template <typename T = void> T Execute(Il2CppList<GameEntity*>* entities) {
		return ((T (*)(ClientWorkerUpdateBawlSystem*, Il2CppList<GameEntity*>*))(Il2CppBase() + 0xE220AC))(this, entities);
	}

};

