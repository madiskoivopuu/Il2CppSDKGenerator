#pragma once
#include <Il2Cpp/Il2Cpp.h>

class SendJsond29
{
public: 

	static Il2CppClass *StaticClass() {
		return (Il2CppClass *)(Il2Cpp::GetClass("Assembly-CSharp.dll", "", "<SendJson>d__29"));
	}

	template <typename T = int32_t> T& 1__state() {
		return *(T*)((uintptr_t)this + 0x0);
	}
	template <typename T = uintptr_t> T& t__builder() {
		return *(T*)((uintptr_t)this + 0x8);
	}
	template <typename T = BugReportWindow*> T& 4__this() {
		return *(T*)((uintptr_t)this + 0x28);
	}
	template <typename T = Il2CppString*> T& json() {
		return *(T*)((uintptr_t)this + 0x30);
	}
	template <typename T = Il2CppString*> T& url() {
		return *(T*)((uintptr_t)this + 0x38);
	}
	template <typename T = UnityCommonLogger*> T& logger() {
		return *(T*)((uintptr_t)this + 0x40);
	}
	template <typename T = uintptr_t> T& request5__2() {
		return *(T*)((uintptr_t)this + 0x48);
	}
	template <typename T = Il2CppObject*> T& u__1() {
		return *(T*)((uintptr_t)this + 0x50);
	}

	template <typename T = void> T MoveNext() {
		return ((T (*)(SendJsond29*))(Il2CppBase() + 0xEEE400))(this);
	}
	template <typename T = void> T SetStateMachine(uintptr_t stateMachine) {
		return ((T (*)(SendJsond29*, uintptr_t))(Il2CppBase() + 0xEEEB94))(this, stateMachine);
	}

};

