#pragma once
#include <Il2Cpp/Il2Cpp.h>

class StartRemoved38
{
public: 

	static Il2CppClass *StaticClass() {
		return (Il2CppClass *)(Il2Cpp::GetClass("Assembly-CSharp.dll", "", "<StartRemove>d__38"));
	}

	template <typename T = int32_t> T& 1__state() {
		return *(T*)((uintptr_t)this + 0x0);
	}
	template <typename T = uintptr_t> T& t__builder() {
		return *(T*)((uintptr_t)this + 0x8);
	}
	template <typename T = ManualQuestTrackerView*> T& 4__this() {
		return *(T*)((uintptr_t)this + 0x28);
	}
	template <typename T = Action1Content1Data*>*>*> T& onComplete() {
		return *(T*)((uintptr_t)this + 0x30);
	}
	template <typename T = TaskAwaiter1IAwaitable1Il2CppString*>*>*> T& u__1() {
		return *(T*)((uintptr_t)this + 0x38);
	}

	template <typename T = void> T MoveNext() {
		return ((T (*)(StartRemoved38*))(Il2CppBase() + 0x1164A88))(this);
	}
	template <typename T = void> T SetStateMachine(uintptr_t stateMachine) {
		return ((T (*)(StartRemoved38*, uintptr_t))(Il2CppBase() + 0x1164D8C))(this, stateMachine);
	}

};

